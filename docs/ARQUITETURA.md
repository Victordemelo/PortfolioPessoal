...
| Barra lateral | `components/Lateral.tsx` | Perfil, "Características", índice e redes. Na página inicial, o índice lista as seções; na página de um projeto, lista as partes do projeto e os outros projetos. Embaixo, as conquistas do GitHub (`data/conquistas.ts`, emblemas em `public/conquistas/`). Fixa (`sticky`) no desktop. |
| Índice ativo | `useSecaoAtiva` em `App.tsx` | Na rolagem, a seção ativa é a última cujo topo passou de 35% da tela; encostou no fim da página, acende a última. Serve para o início e para a página do projeto. Voltando de um projeto, a página cai na seção Projetos. |
